...
我是姜前辰, 毕业于福州大学计算机与大数据学院, 曾担任校超算团队负责人, 获得过ASC世界大学生超级计算机竞赛一等奖等荣誉。现就职于太初电子科技有限公司的推理算子组, 主要负责在异构众核国产芯片上开发大模型推理算子, 参与完成了国产芯片上 vLLM 推理框架相关算子的开发，并进行了主流新模型的算子适配工作。希望能有更多机会学习新知识并将想法付诸实践。
...
- *CUDA 算子融合*: 利用 Nsght-system 和 Nsight-computer 工具分析 CG对角化计算程序热点, 将单 kernel 小算子及访存瓶颈模块采用算子融合的方式优化。
...
- 紧跟 LLM 模型的技术进展, 在异构众核芯片上完成如 DeepSeek 模型 MLA 算子的开发与接入, 如 GPT-OSS 模型相关的 MXFP4 量化权重计算, sliding-window&sinks 等相关模块开发。
